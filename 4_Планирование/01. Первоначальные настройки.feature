...
Сценарий: 01.06 Создание способов закупки
			
	Если не существует элемент справочника 'СпособыВыбораПоставщика' с "Наименование" равным "Запрос котировок" Тогда
		И Я открываю основную форму списка справочника "СпособыВыбораПоставщика"
		И я нажимаю на кнопку с именем 'ФормаСоздать'
		Тогда открылось окно "новый: Способы закупок (создание)"
		И в поле 'Наименование' я ввожу текст 'Запрос котировок'
		И я перехожу к следующему реквизиту
		И в поле 'Код ЕИС' я ввожу текст '3'
		И я перехожу к следующему реквизиту
		И из выпадающего списка с именем 'МетодОценки' я выбираю точное значение 'По ценам'
		И я перехожу к следующему реквизиту
		И из выпадающего списка с именем 'СпособыВыбораПоставщика' я выбираю точное значение 'Запрос котировок'
		И я перехожу к следующему реквизиту
		И я нажимаю на кнопку 'Записать и закрыть'
		И я жду закрытия окна 'новый: Способы закупок (создание) *' в течение 20 секунд
		И я закрываю окно "Способы закупок"
