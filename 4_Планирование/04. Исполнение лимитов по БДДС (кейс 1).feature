...
Сценарий: 04.01.2 Создаем Заявку на оплату (по договору без графика расчетов)				
	...
	* Заполнить аналитики учета и планирования
		...
		ИначеЕсли '$$ЭтоЕРПУХ$$' Тогда
			...
			И в таблице 'РасшифровкаПлатежа' из выпадающего списка с именем 'РасшифровкаПлатежаСтавкаНДС' я выбираю точное значение "20%"
...
Сценарий: 04.01.4 Отражаем фактические данные об оплате	 		 		  		 		 		 	 
	...
	* Проверить движения ОФД	
		...
		ИначеЕсли '$$ЭтоЕРПУХ$$' Тогда
			Тогда таблица "ОперацииБюджетов" стала равной по шаблону:
				| 'N' | 'Период'             | 'Вид бюджета'                      | 'Объект планирования'                                                 | 'Состояние' | 'Аналитика структуры'             | 'Аналитика статей бюджетов'                                   | 'Аналитика партнеров'                                      | 'Приход расход' | 'Валюта' | 'Валюта лимитирования' | 'Сумма'     | 'Лимит'     | 'Сумма исполнено' | 'Лимит исполнено' | 'Источник версии' | 'Курс задан в операции' | 'Курс планирования' | 'Кратность планирования' | 'Курс лимитирования' | 'Кратность лимитирования' | 'Корректировка документа' |
				| '1' | '$ДатаДок$ 0:00:00' | 'Бюджет движения денежных средств' | 'Заявка на оплату *; 3 000 RUB *; ВА - Поставщик1; Оплата поставщику' | ''          | 'ВА - ЦФО 1 (АХО); ВА - ООО Тест' | 'ВА - Статья 1 (ВА - Здания;  - ;  - ;  - ;  - ;  - ) Расход' | 'ВА - ООО Тест; ВА - Поставщик1; 04.01.2 от ; 04.01.2 от ' | 'Расход'        | 'RUB'    | 'RUB'                  | '-3 000,00' | '-3 000,00' | '3 000,00'        | '3 000,00'        | ''                | 'Нет'                   | '1,0000'            | '1'                      | '1,0000'             | '1'                       | ''                        |
...
//		Тогда таблица "КроссТаблица" стала равной по шаблону:
