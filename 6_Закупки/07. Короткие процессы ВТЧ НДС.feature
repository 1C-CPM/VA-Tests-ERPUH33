...
Сценарий: 07.01.01 [План закупок - Заказ поставщику, Королевство Севера] План закупок
	...
	* Статья БДДС
		...
		И в таблице 'ТоварыПоПериодам' я активизирую поле с именем 'ПланыДокументаСтатьяБюджета'
		И в таблице 'ТоварыПоПериодам' я выбираю текущую строку
		И в таблице 'ТоварыПоПериодам' я нажимаю кнопку выбора у реквизита с именем 'ПланыДокументаСтатьяБюджета'
		Тогда открылось окно "Статьи движения денежных средств"
		И в таблице 'Список' я перехожу к строке:
			| "Наименование"  | "Вид аналитики 1" |
			| "Статья БДДС 1" | "БДДС-Аналитика1" |
		И в таблице 'Список' я выбираю текущую строку
		Тогда открылось окно "План закупок (создание) *"
		И я перехожу к следующему реквизиту
		И в таблице 'ТоварыПоПериодам' я нажимаю кнопку выбора у реквизита с именем 'ПланыДокументаАналитика1'
	...
	* Статья БДР
		...
		И в таблице 'ТоварыПоПериодам' я активизирую поле с именем 'ПланыДокументаСтатьяБюджетаБДР'
		И в таблице 'ТоварыПоПериодам' я нажимаю кнопку выбора у реквизита с именем 'ПланыДокументаСтатьяБюджетаБДР'
		Тогда открылось окно "Статьи доходов и расходов"
		И в таблице 'Список' я перехожу к строке:
			| "Наименование" | "Вид аналитики 1" |
			| "Статья БДР 1" | "БДР-Аналитика1"  |
		И в таблице 'Список' я выбираю текущую строку
		Тогда открылось окно "План закупок (создание) *"
		И я перехожу к следующему реквизиту
		И в таблице 'ТоварыПоПериодам' я нажимаю кнопку выбора у реквизита с именем 'ПланыДокументаАналитикаБДР1'
		Тогда открылось окно "Произвольный классификатор"
		И в таблице 'Список' я перехожу к строке:
			| "Наименование" | "Ссылка" |
			| "XXX1"         | "XXX1"   |
		...
		//И Я добавляю Статью БДДС в ТЧ "ТоварыПоПериодам", Статья = "Статья БДДС 1"
		//И Я добавляю Статью БДР в ТЧ "ТоварыПоПериодам", Статья = "Статья БДР 1"
...
Сценарий: 07.01.03 [План закупок - Заказ поставщику, Королевство Севера] Заказ поставщику
	...
	* Проверяем ТЧ
		...
		Тогда таблица 'Товары' стала равной:
			| 'Упаковка' | 'Цена'      | 'Номенклатура' | 'Склад'  | 'Сумма с НДС' | 'Ставка НДС' | 'НДС'      | 'Потребитель (ЦФО)'             | 'Место поставки' | 'Количество' | 'Сумма'     |
			| 'кг'       | '10 000,00' | 'Товар1'       | 'Склад1' | '30 000'      | '22%'        | '5 409,84' | 'ЦФО1 Королевство Севера [223]' | 'МестоПоставки1' | '3,000'      | '30 000,00' |
			| 'упак'     | '8 500,00'  | 'Товар2'       | 'Склад1' | '42 500'      | '22%'        | '7 663,93' | 'ЦФО1 Королевство Севера [223]' | 'МестоПоставки1' | '5,000'      | '42 500,00' |
