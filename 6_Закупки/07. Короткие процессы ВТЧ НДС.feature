...
Сценарий: 07.01.03 [План закупок - Заказ поставщику, Королевство Севера] Заказ поставщику
	...
	* Проверяем ТЧ
		...
		Тогда таблица 'Товары' стала равной:
			...
			| 'кг'       | '10 000,00' | 'Товар1'       | 'Склад1' | '30 000'      | '20%'        | '5 000,00' | 'ЦФО1 Королевство Севера [223]' | 'МестоПоставки1' | '3,000'      | 'Статья БДР 1;XXX1;YYY1;ZZZ1;SSS1;RRR1;WWW1;Статья БДДС 1;AAA1;BBB1;CCC1;DDD1;EEE1;FFF1' | '30 000,00' |
			| 'упак'     | '8 500,00'  | 'Товар2'       | 'Склад1' | '42 500'      | '20%'        | '7 083,33' | 'ЦФО1 Королевство Севера [223]' | 'МестоПоставки1' | '5,000'      | 'Статья БДР 1;XXX1;YYY1;ZZZ1;SSS1;RRR1;WWW1;Статья БДДС 1;AAA1;BBB1;CCC1;DDD1;EEE1;FFF1' | '42 500,00' |
...
//		Тогда открылось окно "Договор с поставщиком / исполнителем (создание) *"
	...
	* Проверка правильности подбора номенклатуры из плана закупок
		...
		Тогда таблица 'Номенклатура' стала равной:
			| 'Цена'       | 'Единица измерения' | 'Страна производителя' | 'Номенклатура' | 'Количество' | 'Место поставки' | 'Дата поставки' | 'ЦФО'                   | 'Сумма без НДС' | 'Ставка НДС' | 'Проект'  | 'Сумма с НДС'  | 'Сумма НДС'  |
			| '101 980,00' | 'шт'                | ''                     | 'Товар6'       | '10,000'     | 'МестоПоставки1' | '31.01.2026'    | 'ЦФО1 Королевство Скал' | '849 833,33'    | '20%'        | 'Проект3' | '1 019 800,00' | '169 966,67' |
			| '101 980,00' | 'шт'                | ''                     | 'Товар6'       | '10,000'     | 'МестоПоставки1' | '28.02.2026'    | 'ЦФО1 Королевство Скал' | '849 833,33'    | '20%'        | 'Проект3' | '1 019 800,00' | '169 966,67' |
...
Сценарий: 07.03.01 [План закупок - Лот, Королевство Скал] Закупочная процедура
	...
	* Лот
		...
		* Проверяем подобранные товары
		...
		Тогда таблица 'Номенклатура' стала равной:
			| 'Цена'       | 'Единица измерения' | 'Номенклатура' | 'Дата поставки' | 'Коэффициент' | 'Период потребности' | 'Ставка НДС' | 'Сумма без НДС' | 'Цена первоначальная' | 'Сумма НДС'  | 'ЦФО'                   | 'Сумма с НДС'  | 'Проект'  | 'Место поставки' | 'Количество' |
			| '101 980,00' | 'шт'                | 'Товар6'       | '31.12.2026'    | '1,00000000'  | 'Декабрь 2026 г.'    | '20%'        | '849 833,33'    | '101 980,00'          | '169 966,67' | 'ЦФО1 Королевство Скал' | '1 019 800,00' | 'Проект3' | 'МестоПоставки1' | '10,000'     |
