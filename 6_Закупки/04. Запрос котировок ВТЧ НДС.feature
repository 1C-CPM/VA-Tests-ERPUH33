...
Функционал: 04. Запрос котировок
...
Сценарий: 04.00. Иинициализация переменных
	...
	И Я запоминаю значение выражения 'Ложь' в переменную "Счетчик"	
...
//		И из выпадающего списка с именем "СпособВыбораПоставщика" я выбираю точное значение 'Запрос котировок'
	...
	* Предмет договора	
		...
		И в поле с именем 'InputFld' я ввожу текст 'Приобретение материалов, тест 04'
	...
	* Общие требования
		...
		И в поле с именем 'InputFld' я ввожу текст 'Нет, тест 04'
		И я нажимаю на кнопку с именем 'OK'
		Тогда открылось окно 'Строка плана закупок (создание) *'
	...
	* 2026
		...
		И Я добавляю Номенклатуру в ТЧ ="Номенклатура", Номенклатура = "Материал1", ЦФО = "Королевство Севера [223]", МестоПоставки = "МестоПоставки1", ДатаПоставки = "15.08.2026", Колво = "100", Цена = "1 200,00", СтавкаНДС = "20%", Проект = "Проект1"
	...
	* 2027
		...
		И Я добавляю Номенклатуру в ТЧ ="Номенклатура", Номенклатура = "Материал2", ЦФО = "Королевство Севера [223]", МестоПоставки = "МестоПоставки1", ДатаПоставки = "20.07.2027", Колво = "250", Цена = "1 400,00", СтавкаНДС = "20%", Проект = "Проект1"
		И Я добавляю Статью БДДС в ТЧ "Номенклатура", Статья = "Статья БДДС 1", А1 = "AAA1", А2 = "BBB1", А3 = "CCC1", А4 = "DDD1", А5 = "EEE1", А6 = "FFF1"
		И Я добавляю Статью БДР в ТЧ "Номенклатура", Статья = "Статья БДР 1", А1 = "XXX1", А2 = "YYY1", А3 = "ZZZ1", А4 = "SSS1", А5 = "RRR1", А6 = "WWW1"
	...
	* Запись документа
		...
		И Я закрываю окно "Строки планов закупок (223-ФЗ)"
...
Сценарий: 04.02. [ЗАПРОС КОТИРОВОК] Закупочная процедура 1 / лотовая
	...
	* Создаем процедуру
		...
		И в таблице "Список" я перехожу к строке:
			...
			|'2026 г.'       | 'Королевство Севера [223]' | 'Семь королевств' | 'Запрос котировок' | 'Приобретение материалов, тест 04' | 'Утвержден' | '470 000,00' |  'RUB'  | 'Утвержден' |
	...
	* Способ закупки, метод оценка, тип закупки
		...
		И я выбираю из списка 'По конкурентным листам'
...
//		Тогда открылось окно 'Закупочная процедура №*'
	...
	* Настройка оценки: общие критерии
	...
	* Настройка оценки: критерии по номенклатуре
...
Сценарий: 04.03. [ЗАПРОС КОТИРОВОК] Предложение 1
		...
		И в таблице "Список" я перехожу к строке:
			...
			| 'Семь королевств'     |'Королевство Севера [223]'| '2026 г.'        | 'Приобретение материалов, тест 04' | 'Запрос котировок' |'Утвержден' |
		И в таблице "Список" я выбираю текущую строку
		Тогда открылось окно 'Закупочная процедура №*'
		...
		И в таблице "Лоты" я перехожу к строке:
			...
			| '2026 г.'        | 'Лотовая закупка' | 'Королевство Севера [223]' | 'По конкурентным листам' | 'Приобретение материалов, тест 04' | '470 000,00, RUB' |
		...
		* Удаляем график оплаты из лота

		И в таблице 'ЭтапыГрафикаОплаты' я активизирую поле с именем 'ЭтапыГрафикаОплатыВариантОтсчета'
		И в таблице 'ЭтапыГрафикаОплаты' я удаляю строку
		* Вводим график оплаты от поставщика
...
Сценарий: 04.04. [ЗАПРОС КОТИРОВОК] Предложение 2
		...
		* Критерии по номенклатуре Материал 1
		...
		И в таблице 'ОценкаПоКритериямПозицииНоменклатур' я перехожу к строке:
			| "Критерий оценки"  |
			| "Условия поставки" |
		И в таблице 'ОценкаПоКритериямПозицииНоменклатур' в поле с именем 'ОценкаПоКритериямПозицииНоменклатурЗначение' я ввожу текст "РЖД"
		И в таблице 'ОценкаПоКритериямПозицииНоменклатур' я завершаю редактирование строки
...
Сценарий: 04.06. [ЗАПРОС КОТИРОВОК] Договор ДД1
	...
	* Заполняем договор
		...
		И элемент формы 'Условие оплаты' стал равен 'Этапов оплаты: 2, по календарным дням'
		...
		Тогда таблица 'Номенклатура' стала равной:
			| 'Место поставки'  | 'Цена'     | 'Страна производителя' | 'Единица измерения' | 'Номенклатура' | 'ЦФО'                      | 'Страны происхождения' | 'Сумма без НДС' | 'Ставка НДС' | 'Проект'  | 'Сумма с НДС' | 'Сумма НДС' | 'Аналитика 1' | 'Количество' | 
			| 'МестоПоставки1'  | '1 000,00' | 'РОССИЯ'               | 'т'                 | 'Материал1'    | 'Королевство Севера [223]' | 'РОССИЯ'               | '83 333,33'     | '20%'        | 'Проект1' | '100 000,00'  | '16 666,67' | 'XXX1'        | '100,000'    | 
			| 'МестоПоставки1'  | '1 200,00' | 'РОССИЯ'               | 'т'                 | 'Материал2'    | 'Королевство Севера [223]' | 'РОССИЯ'               | '250 000,00'    | '20%'        | 'Проект1' | '300 000,00'  | '50 000,00' | 'XXX1'        | '250,000'    | 
...
Сценарий: 04.07. [ЗАПРОС КОТИРОВОК] ПТУ по договору ДД1
	...
	* Проверяем правильность заполнения строки
		...
		И в таблице "Товары" текущая строка равна:
			| 'Номенклатура' | 'Ставка НДС' | 'Количество' | 'Ед. Изм.' | 'Цена'     | 'НДС'       | 'Страна происхождения'  | 'Сумма'      |
			| 'Материал1'    | '20%'        | '100,000'    | 'т'        | '1 000,00' | '16 666,67' | 'РОССИЯ'                | '100 000,00' |
//			
//		И в таблице "Товары" я перехожу к строке:
//			| 'Номенклатура' |
//			| 'Материал2'    |
