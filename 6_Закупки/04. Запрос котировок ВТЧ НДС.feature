...
Сценарий: 04.02. [ЗАПРОС КОТИРОВОК] Закупочная процедура 1 / лотовая
	...
	* Создаем процедуру
		...
		Тогда открылось окно 'Строки планов закупок (223-ФЗ)'
		И в таблице "Список" я перехожу к строке:
			|'Период закупок'| 'Заказчик'                 | 'Организатор'     | 'Способ закупки'   | 'Предмет договора'                 | 'Сумма'      |'Валюта' |
			|'2026 г.'       | 'Королевство Севера [223]' | 'Семь королевств' | 'Запрос котировок' | 'Приобретение материалов, тест 04' | '470 000,00' |  'RUB'  |
...
Сценарий: 04.06. [ЗАПРОС КОТИРОВОК] Договор ДД1
	...
	* Заполняем договор
		...
		Тогда таблица 'Номенклатура' стала равной:
			| 'Место поставки'  | 'Цена'     | 'Страна производителя' | 'Единица измерения' | 'Номенклатура' | 'ЦФО'                      | 'Страны происхождения' | 'Ставка НДС' | 'Проект'  | 'Сумма'      | 'Сумма НДС' | 'Аналитика 1' | 'Количество' | 
			| 'МестоПоставки1'  | '1 000,00' | 'РОССИЯ'               | 'т'                 | 'Материал1'    | 'Королевство Севера [223]' | 'РОССИЯ'               | '22%'        | 'Проект1' | '100 000,00' | '18 032,79' | 'XXX1'        | '100,000'    | 
			| 'МестоПоставки1'  | '1 200,00' | 'РОССИЯ'               | 'т'                 | 'Материал2'    | 'Королевство Севера [223]' | 'РОССИЯ'               | '22%'        | 'Проект1' | '300 000,00' | '54 098,36' | 'XXX1'        | '250,000'    | 
...
Сценарий: 04.07. [ЗАПРОС КОТИРОВОК] ПТУ по договору ДД1
	...
	* Дата
		...
		И в поле с именем 'Дата' я ввожу текст "01.09.2026  0:00:00"
		И я перехожу к следующему реквизиту
	...
	* Проверяем правильность заполнения строки
		...
		И в таблице "Товары" текущая строка равна:
			| 'Номенклатура' | 'Ставка НДС' | 'Количество' | 'Упаковка' | 'Цена'     | 'НДС'       | 'Страна производителя'  | 'Сумма'      |
			| 'Материал1'    | '22%'        | '100,000'    | 'т'        | '1 000,00' | '18 032,79' | 'РОССИЯ'                | '100 000,00' |
	...
	* Закрываем текущий сеанс тестирования
