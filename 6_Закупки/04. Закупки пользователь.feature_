...
Функционал: 04. Тесты по закупкам под "МенеджерПоЗакупкам1"
	...
	Как бизнес-аналитик я хочу что-то сделать
...
Сценарий: 04.07. [ЗАПРОС ПРЕДЛОЖЕНИЙ] ЗП-2(попозиционная)
	...
	* Создаем процедуру
		...
		И в таблице "Список" я перехожу к строке:
			...
			| 'RUB'              | 'Королевство Севера [223]' | 'Семь королевств'     |'2025 г.'        | 'Приобретение материалов, тест 2' | 'Утвержден' | 'Запрос предложений' | 'Утвержден' | '450 000,00'      |
	...
	* Закладка лоты
		...
		И в таблице "Лоты" я перехожу к строке:
			| 'Период закупок' | 'Вид закупки'           | 'Заказчик'                 | 'Метод оценки' | 'Предмет договора'                | 'НМЦ'             |
			| '2025 г.'        | 'Попозиционная закупка' | 'Королевство Севера [223]' | 'По ценам'     | 'Приобретение материалов, тест 2' | '450 000,00, RUB' |
