...
Сценарий: 05.02. [ЗАПРОС ПРЕДЛОЖЕНИЙ] СПЗ 2
	...
	* Предмет договора	
		...
		И в поле с именем 'InputFld' я ввожу текст 'Приобретение материалов, тест 05'
	...
	* Общие требования
		...
		И в поле с именем 'InputFld' я ввожу текст 'Нет, тест 05'
		И я нажимаю на кнопку с именем 'OK'
		Тогда открылось окно 'Строка плана закупок (создание) *'
	...
	* Закладка ЕИС
		...
		И в поле с именем 'ПорядокФормированияЦеныДоговора' я ввожу текст "Цена вклчючает НДС"
	...
	* Материал2
		...
		И Я добавляю Номенклатуру в ТЧ ="Номенклатура", Номенклатура = "Материал2", ЦФО = "ЦФО1 Королевство Простора", МестоПоставки = "МестоПоставки1", ДатаПоставки = "25.03.2026", Колво = "100", Цена = "1 200,00", СтавкаНДС = "22%", Проект = "Проект1"
	...
	* Материал3
		...
		И Я добавляю Номенклатуру в ТЧ ="Номенклатура", Номенклатура = "Материал3", ЦФО = "ЦФО1 Королевство Простора", МестоПоставки = "МестоПоставки3", ДатаПоставки = "18.05.2026", Колво = "150", Цена = "1 500,00", СтавкаНДС = "22%", Проект = "Проект1"
		//И Я добавляю Статью БДДС в ТЧ "Номенклатура", Статья = "Статья БДДС 1", А1 = "AAA1", А2 = "BBB1", А3 = "CCC1", А4 = "DDD1", А5 = "EEE1", А6 = "FFF1"
		//И Я добавляю Статью БДР в ТЧ "Номенклатура", Статья = "Статья БДР 1", А1 = "XXX1", А2 = "YYY1", А3 = "ZZZ1", А4 = "SSS1", А5 = "RRR1", А6 = "WWW1"
		//И я завершаю редактирование строки
...
Сценарий: 05.05. [ЗАПРОС ПРЕДЛОЖЕНИЙ] Закупочная процедура 2 / попозиционная
	...
	* Создаем процедуру
		...
		И в таблице "Список" я перехожу к строке:
			...
			| 'RUB'    | 'Королевство Дорна [223]' | 'Семь королевств' |'2026 г.'        | 'Приобретение материалов, тест 05' | 'Запрос предложений' | 'Утвержден' | '345 000,00' |
	...
	* Условия
		...
		И в таблице 'ЭтапыГрафикаОплаты' я выделяю все строки
		И в таблице 'ЭтапыГрафикаОплаты' я нажимаю на кнопку с именем 'ЭтапыГрафикаОплатыУдалить'
										
		И в таблице 'ЭтапыГрафикаОплаты' я нажимаю на кнопку с именем 'ЭтапыГрафикаОплатыДобавить'
		И в таблице 'ЭтапыГрафикаОплаты' в поле с именем 'ЭтапыГрафикаОплатыСдвиг' я ввожу текст "30"
		И я перехожу к следующему реквизиту
...
//			И я нажимаю на кнопку с именем 'Button0'
	...
	* Прочие условия
		...
		И в поле с именем 'ПорядокОценкиИСопоставленияЗаявок' я ввожу текст "Порядок оценки и сроки рассмотрения заявок"
		И я перехожу к следующему реквизиту
...
Сценарий: 05.08. [ЗАПРОС ПРЕДЛОЖЕНИЙ] Предложение 1
		...
		И в таблице "Список" я перехожу к строке:
			...
			| 'Семь королевств'     |'Королевство Дорна [223]' | '2026 г.'        | 'Приобретение материалов, тест 05' | 'Запрос предложений' |'Утвержден' |
		И в таблице "Список" я выбираю текущую строку
		Тогда открылось окно 'Закупочная процедура №*'
		...
		И в таблице "Лоты" я перехожу к строке:
			...
			| '2026 г.'        | 'Лотовая закупка' | 'Королевство Дорна [223]' | 'По критериям (Настройка завершена)' | 'Приобретение материалов, тест 05' | '345 000,00, RUB' |
...
Сценарий: 05.08. [ЗАПРОС ПРЕДЛОЖЕНИЙ] Протокол
	...
	* Результат
		...
		И в поле с именем 'РезультатРаботыКомиссии' я ввожу текст "#5 Победитель определен, кворум имеется"
		И я перехожу к следующему реквизиту
